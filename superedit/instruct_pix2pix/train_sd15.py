            ...
            with accelerator.accumulate(accumulate_params):
                ...
                if args.neg_edit_prompt_column is not None:
                    ...
                    if args.stop_pos_grad:
                        pos_distance = torch.norm(target.float() - model_pred_pos.float().detach(), p=2, dim=[1,2,3])
                    else:
                        pos_distance = torch.norm(target.float() - model_pred_pos.float(), p=2, dim=[1,2,3])
                    # negative distance calculation
                    neg_distance = torch.norm(target.float() - model_pred_neg.float(), p=2, dim=[1,2,3])
                    # triplet loss
                    ...
                    triplet_loss = (triplet_loss * triplet_loss_mask).sum() / triplet_loss_mask.sum()
                    triplet_loss = torch.tensor(0.0, device=triplet_loss.device) if prompt_mask.sum() == 0 else triplet_loss
